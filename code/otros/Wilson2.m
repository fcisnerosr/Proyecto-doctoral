...
function [u,v,ac,u00,v00,ac00]=Wilson2(dt,TDuration,Ma,x,fi,forcet,omega,u00,v00,massv)
...
Forcep=zeros(size(forcet));
forcetm=forcet-Forcep;
i=0;

while abs(sum(sum(Forcep))-sum(sum(forcetm)))/abs(sum(sum(forcetm)))>0.01
