...
function [React, DeltaGo,Fi]=SAPSMA(pathfile)
...
Preact=zeros(IDmin-IDmax,1);

% assamblage of node forces
% assamblage of node forces
if isempty(nodeforces)==0
for j=1:length(nodeforces(:,1))
    for i=1:length(nodeforces(1,2:end))
LVnodes(i,j)=[ID(i,nodeforces(j,1))];
if LVnodes(i,j)>0
Pnodes(LVnodes(i,j))=nodeforces(j,i+1);
else
 Preact(-LVnodes(i,j)-IDmax)=nodeforces(j,i+1);   
end
    end
end
end
...
disp('Static analysis of sn forces has finished');
