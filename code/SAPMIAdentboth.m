...
%% Datos iniciales de entrada
    ...
    no_elemento_a_danar = [2, 4, 5, 6, 21];
    caso_dano           = {'corrosion', 'corrosion', 'corrosion', 'corrosion', 'corrosion'};
    dano_porcentaje     = [50, 50, 50, 50, 50];
...
% %% Gráfica de formas modales
%     graf_formas_modales(modos_cond, nodes)
...
%     modos_und_norm = [];
%     for i = 1:3
%         maximo          = max(abs(modos_undamage_cond), [], 1);
%         vector_norm_und = modos_undamage_cond(:,i) / maximo(i);
%         modos_und_norm  = [modos_und_norm, vector_norm_und];
%     end
% 
%     % Con daño
%     modos_dam_norm = [];
%     for i = 1:3
%         maximo          = max(abs(modos_damage_cond), [], 1);
%         vector_norm_dam = modos_damage_cond(:,i) / maximo(i);
%         modos_dam_norm  = [modos_dam_norm, vector_norm_dam];
%     end
% 
%     % Diferencias
%     dif_1er_modo = abs(modos_und_norm(:, 1) - modos_dam_norm(:, 1));
%     [max_valor, indice_maximo] = max(dif_1er_modo)
%     dif_2do_modo = abs(modos_und_norm(:, 2) - modos_dam_norm(:, 2));
%     [max_valor, indice_maximo] = max(dif_2do_modo)
%     dif_3er_modo = abs(modos_und_norm(:, 3) - modos_dam_norm(:, 3));
%     [max_valor, indice_maximo] = max(dif_3er_modo)

    % hrz_dif_todos_modos = horzcat(dif_1er_modo, dif_2do_modo, dif_3er_modo)
toc
