...
% % % 4. Perc (Perc no aplica para las formas modales)
% % Perc = abs(modos_cond_d - modos_cond_u) ./ abs(modos_cond_u);
% % Perc_min = min(Perc(:)); 
% % Perc_max = max(Perc(:));
% % Perc_norm = (Perc - Perc_min) ./ (Perc_max - Perc_min);         % Perc normalizado
% 
% % Flexibility
...
for j = 1:nNodes
    P(j) = w1 * DI1_COMAC(j) + w2 * DI2_Diff(j) + w3 * DI3_Div(j);
end
