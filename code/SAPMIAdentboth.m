...
%% SECCIÓN: pestaña "nudos"
    ...
    % SUBLOQUE: Diámetros y espesores de los elementos elementos tubulares de la superestructura (vector_diametros_tube_super y vector_thickness__tube_super)
    hoja_excel = 'Frame Assigns - Summary';
    tabla_frame_assigns_summary = readtable(archivo_excel, 'Sheet', hoja_excel);
    tabla_frame_assigns_summary(1,:) = [];
    tabla_a_filtrar = readtable(archivo_excel, 'Sheet', hoja_excel);
    ...
    % prop_geom = num2cell(z prop_geom);
    ...
    % E_columna = num2cell(E_columna);
    % G_columna = num2cell(G_columna);
    % prop_geom = horzcat(prop_geom, E_columna, G_columna, tipo, wo_vector, gamma_beta_vector);
    % prop_geom(:,1) = [];
    % numero_frame_cell = num2cell(numero_frame);
    % prop_geom = horzcat(numero_frame_cell, prop_geom)
