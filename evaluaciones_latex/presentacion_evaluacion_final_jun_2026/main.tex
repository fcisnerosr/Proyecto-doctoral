...
\begin{frame}{Análisis Estadístico: 3 Puntos Clave}
    \centering
    \large
    \vspace{0.5cm}
    ...
    \begin{enumerate}
        ...
        \item[\textbf{P2}] \textbf{Robustez al Ruido AWGN} \\
        \small Desempeño del ICD bajo condiciones experimentales ruidosas (SNR variables)

        \vspace{0.5cm}

        \item[\textbf{P3}] \textbf{Métricas de Clasificación y Hungarian Algorithm} \\
...
\begin{frame}{P1: Curva POD — ¿Qué es y cómo se calcula?}
    ...
    \begin{block}{Comparación}
        Se compara la curva POD del \textbf{ICD fusionado} contra el \textbf{mejor DI individual} ($DI_6$, identificado por máxima AUC-ROC). \textit{Una curva más alta = el método detecta daños con mayor probabilidad.}
    \end{block}
\end{frame}
...
\begin{frame}{P2: Robustez al Ruido AWGN$^*$ (Descripción)}
    \small
    ...
    \vspace{0.15cm}
    {\tiny $^*$AWGN: \textbf{A}dditive \textbf{W}hite \textbf{G}aussian \textbf{N}oise — Ruido que se suma a la señal original, con espectro plano y distribución normal (Shannon, 1948). Estándar en teoría de comunicaciones e ingeniería de confiabilidad.}
\end{frame}

\begin{frame}{P2: Fórmula — SNR y Degradación}
    \small
...
\begin{frame}{P3: Hungarian Algorithm y Métricas de Clasificación (Descripción)}
    \small
...
\begin{frame}{P3: Fórmula — Hungarian Algorithm}
    \small
...
\begin{frame}{P3: Gráficas — Métricas de Clasificación (Corrosión)}
    \centering
    \vspace{0.2cm}
    \includegraphics[width=0.85\textwidth, height=0.8\textheight, keepaspectratio]{P5_metrics_corrosion.png}
\end{frame}

\begin{frame}{P3: Gráficas — Métricas de Clasificación (Abolladura)}
    \centering
    \vspace{0.2cm}
