...
for i=1:NE
 ...
 % local stiffness matrix of the elements
     ...
     h=sqrt(12*Iy(i)/A(i));
     b=A(i)/h;
     depthc=depthr(locdam)*h;
        ke(:,:,i)=Zhengc8(L(i),xdc,A(i),Iz(i),Iy(i),J(i),E(i),locdam,depthc,G(i),h,b); 
        ke0=localkeframe3D(A(i),Iy(i),Iz(i),J(i),E(i),G(i),L(i));
        kedif(:,:,locdam)=ke0-ke(:,:,i);
 end
...
% for j=1:mr
%     if ID(1,rigid(j,2))>0
%         KGr(:,ID(1,rigid(j,1)))=KG(:,ID(1,rigid(j,1)))+KG(:,ID(1,rigid(j,2)));
%         KGr(:,ID(5,rigid(j,1)))=KG(:,ID(5,rigid(j,1)))+KG(:,ID(1,rigid(j,2)))*(nodes(rigid(j,2),4)-nodes(rigid(j,1),4));
%         KGr(:,ID(6,rigid(j,1)))=KG(:,ID(6,rigid(j,1)))-KG(:,ID(1,rigid(j,2)))*(nodes(rigid(j,2),3)-nodes(rigid(j,1),3));
%         KGr(:,ID(2,rigid(j,1)))=KG(:,ID(2,rigid(j,1)))+KG(:,ID(2,rigid(j,2)));
%         KGr(:,ID(4,rigid(j,1)))=KG(:,ID(4,rigid(j,1)))-KG(:,ID(2,rigid(j,2)))*(nodes(rigid(j,2),4)-nodes(rigid(j,1),4));
%         KGr(:,ID(6,rigid(j,1)))=KGr(:,ID(6,rigid(j,1)))+KG(:,ID(2,rigid(j,2)))*(nodes(rigid(j,2),2)-nodes(rigid(j,1),2));
%         KGr(:,ID(3,rigid(j,1)))=KG(:,ID(3,rigid(j,1)))+KG(:,ID(3,rigid(j,2)));
%         KGr(:,ID(4,rigid(j,1)))=KGr(:,ID(4,rigid(j,1)))+KG(:,ID(3,rigid(j,2)))*(nodes(rigid(j,2),3)-nodes(rigid(j,1),3));
%         KGr(:,ID(5,rigid(j,1)))=KGr(:,ID(5,rigid(j,1)))-KG(:,ID(3,rigid(j,2)))*(nodes(rigid(j,2),2)-nodes(rigid(j,1),2));
%         KGr(:,ID(4,rigid(j,1)))=KGr(:,ID(4,rigid(j,1)))+KG(:,ID(4,rigid(j,2)));
%         KGr(:,ID(5,rigid(j,1)))=KGr(:,ID(5,rigid(j,1)))+KG(:,ID(5,rigid(j,2)));
%         KGr(:,ID(6,rigid(j,1)))=KGr(:,ID(6,rigid(j,1)))+KG(:,ID(6,rigid(j,2)));
%         end
% end
% IDr=ID;
...
% for j=1:mr
%     if ID(1,rigid(j,2))>0
%         indxIDl=find(ID(:,rigid(j,2))>0);
%         IDr(indxIDl,rigid(j,2))=ID(indxIDl,rigid(j,2))*(-1);
%      end
% end
% 
% DOFr=find(IDr>0)-IDmax;
% 
% KG=KGr(DOFr,DOFr);
...
% mass matrix
masses= massele(nodes,elements,L,A,gammae);
%masses= masselement(nodes,elements,L,A,tipo,water,radio,b,h,gammae);
...
g=9810;        % gravity constant

    massv=abs(force)/g;
    

[u,v,ac]=Wilson2(dt,TDuration,Mass,phi,fi,force,omega,u0,v0,massv);
...
results='MPx MPy MPz DeltaG DeltaGo Fi React KG ke kg Pnodes Mass fn LT ID L CX CY CZ FEMG NForce force dt TDuration phi phio Tn u v ac ;';
